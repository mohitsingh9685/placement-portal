...
4. **My applications:** each collapsed card shows the company, status/current round and a Revoke application button for active applications. Click the company/status header (or press Enter/Space) to expand the role, timeline, submitted details, original resume version, documents and recruitment plan; opening another card collapses the first. Revoke application opens and focuses the withdrawal reason form, and sending it requires placement-team approval. Pending requests disable the shortcut; withdrawn/rejected applications have no revoke button. Notification links open the matching application automatically. Later profile edits do not silently change an application. Legacy records state when earlier history is unavailable.
