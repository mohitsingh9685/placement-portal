...
export default function CompanyFilters({ filters, onChange, onReset, admin = false, guest = false, compact = false }) {
  ...
  return <section aria-label="Company filters" className={`rounded-2xl border border-white/10 bg-slate-900/80 ${compact ? "p-4 text-sm" : "p-5"}`}><div className={`grid sm:grid-cols-2 lg:grid-cols-4 ${compact ? "gap-3 2xl:grid-cols-8" : "gap-4"}`}>
    ...
    {onReset && <div className="flex items-end sm:justify-end"><ResetFiltersButton onClick={onReset} className={`w-full sm:w-auto ${compact ? "min-h-10" : "min-h-12"}`} /></div>}
