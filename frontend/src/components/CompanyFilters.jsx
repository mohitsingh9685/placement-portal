...
export default function CompanyFilters({ filters, onChange, onReset, admin = false, guest = false, compact = false, sidebar = false }) {
  const field = `block w-full min-w-0 rounded-xl border border-white/15 bg-slate-950 ${compact ? "mt-1 px-3 py-2 text-sm" : "mt-2 p-3"}`;
  ...
  return <section aria-label="Company filters" className={`rounded-2xl border border-white/10 bg-slate-900/80 ${compact ? "p-4 text-sm" : "p-5"}`}>
    {sidebar && <div className="mb-5 flex items-center justify-between gap-2"><h2 className="font-semibold text-slate-100">Filter drives</h2>{onReset && <ResetFiltersButton onClick={onReset} className="!px-3 !py-1.5 !text-xs" />}</div>}
    <div className={`grid ${sidebar ? "gap-4 sm:grid-cols-2 lg:grid-cols-1" : `sm:grid-cols-2 lg:grid-cols-4 ${compact ? "gap-3 2xl:grid-cols-8" : "gap-4"}`}`}>
