    ...
    <div ref={body} className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
      ...
      <fieldset disabled={busy} className={`min-w-0 ${tab === "details" ? "h-full" : ""}`}><div role="tabpanel" id={`role-panel-${tab}`} aria-labelledby={`role-tab-${tab}`} className={tab === "details" ? "h-full" : ""}>
        ...
        {tab === "eligibility" && <div className="space-y-5">
          <div className="grid gap-4 sm:grid-cols-3">{[["minCgpa", "Minimum CGPA", 10, "0.01"], ["minTenthPercentage", "10th cutoff %", 100, "0.01"], ["minTwelfthPercentage", "12th cutoff %", 100, "0.01"], ["minDiplomaPercentage", "Diploma cutoff %", 100, "0.01"], ["maxTotalBacklogs", "Total backlog limit", 100, "1"]].map(([key, label, max, step]) => <Field key={key} label={label} field={key} type="number" min="0" max={max} step={step} value={role.eligibility[key]} onChange={v => eligibility(key, v)} placeholder="No limit" />)}
          ...
          <p className="text-xs text-slate-400">12th or diploma cutoff applies to the student's entry qualification.</p>
