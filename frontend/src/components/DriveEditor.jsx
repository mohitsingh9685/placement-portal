...
import { newRole, emptyDrive, editorFromGraph, drivePayload } from "../utils/driveEditor";
...
export default function DriveEditor({ id }) {
  ...
  const navigate = useNavigate(), location = useLocation(), main = useRef(null);
  ...
  const [companyTab, setCompanyTab] = useState("details"), [issue, setIssue] = useState(null);
  ...
  function attachments(roleId) {
    ...
    return <DriveDocuments key={roleId || "shared"} compact pageSize={roleId ? 5 : 3} disabledMessage={dirty ? "Save drive changes before managing documents." : "Updating documents…"} companyId={graph._id} documents={target?.attachments} history={target?.retiredAttachments} manage disabled={busy || dirty}
  ...
  return <div className="premium-shell drive-editor-shell min-h-screen lg:flex lg:h-dvh lg:flex-col lg:overflow-hidden"><div className="shrink-0"><Navbar wide /></div>
    <main ref={main} className="flex min-h-0 flex-1 flex-col gap-4 px-4 py-4 text-slate-100 sm:px-6 lg:px-8">
      <header className="flex shrink-0 flex-wrap items-center justify-between gap-3">
        ...
        {(!id || graph) && <div className="flex flex-wrap items-center gap-3">{dirty && <span className="text-xs text-amber-200">Unsaved changes</span>}{graph && <button type="button" className={outline} disabled={busy || dirty} title={dirty ? "Save changes first" : undefined} onClick={() => status(graph.drive.status === "PUBLISHED" ? "CLOSED" : "PUBLISHED")}>{graph.drive.status === "PUBLISHED" ? "Close applications" : graph.drive.status === "CLOSED" ? "Reopen drive" : "Publish drive"}</button>}<button type="submit" form="drive-editor-form" className={editorButton} disabled={busy || !programs.length}>{busy ? "Saving…" : graph ? "Save changes" : "Save draft"}</button></div>}
      </header>
      {(error && !roleOpen) && <p role="alert" className="shrink-0 rounded-lg border border-red-400/25 bg-red-950/60 px-3 py-2 text-sm">{error} <button type="button" className="ml-3 underline" onClick={() => window.location.reload()}>Reload saved version</button></p>}
      ...
      {id && !graph ? <p>Unable to load this drive.</p> : <>
        <form id="drive-editor-form" noValidate onSubmit={save} className="min-h-0 lg:flex lg:flex-1 lg:flex-col">
          <fieldset disabled={busy} className="flex min-h-0 flex-1 flex-col rounded-2xl border border-white/10 bg-slate-900/75">
            <div className="flex shrink-0 items-center justify-between gap-3 border-b border-white/10 px-5 py-3"><h2 className="font-semibold">Company & drive</h2><div className="flex gap-1"><button type="button" className={`rounded-lg px-3 py-1.5 text-xs ${companyTab === "details" ? "bg-cyan-400/10 text-cyan-200" : "text-slate-400"}`} aria-pressed={companyTab === "details"} onClick={() => setCompanyTab("details")}>Details</button><button type="button" className={`rounded-lg px-3 py-1.5 text-xs ${companyTab === "documents" ? "bg-cyan-400/10 text-cyan-200" : "text-slate-400"}`} aria-pressed={companyTab === "documents"} onClick={() => setCompanyTab("documents")}>Shared documents{graph?.drive.attachments?.length ? ` (${graph.drive.attachments.length})` : ""}</button></div></div>
            {companyTab === "details" ? <div className="grid min-h-0 flex-1 gap-5 p-5 lg:grid-cols-[minmax(280px,0.8fr)_minmax(0,1.6fr)]">
              <div className="grid content-start gap-3"><Field label="Company name" field="companyName" maxLength={200} value={form.companyName} onChange={companyName => change({ companyName })} placeholder="Enter company name" /><Field label="Drive title" field="title" maxLength={200} value={form.title} onChange={title => change({ title })} placeholder="Graduate hiring 2027" /><Field label="Application deadline · India time" field="registrationDeadline" type="datetime-local" value={form.registrationDeadline} onChange={registrationDeadline => change({ registrationDeadline })} /><label className="flex items-center gap-2 pt-1 text-sm text-slate-300"><input type="checkbox" checked={Boolean(form.dreamOpportunity)} onChange={e => change({ dreamOpportunity: e.target.checked })} />Dream opportunity</label></div>
            ...
            </div> : <div className="min-h-0 flex-1 overflow-y-auto p-4">{graph ? attachments() : <div className="rounded-xl border border-dashed border-white/15 px-5 py-8 text-center"><h3 className="font-semibold">Shared documents</h3><p className="mt-2 text-sm text-slate-400">Save the drive first, then attach documents for every role.</p></div>}</div>}
          </fieldset>
        </form>
        <section aria-labelledby="drive-roles-heading" className="shrink-0 rounded-2xl border border-white/10 bg-slate-900/75 p-4">
          ...
          </button>; })}{form.roles.length === 0 && <button type="button" onClick={addRole} className="rounded-xl border border-dashed border-cyan-300/25 py-7 text-sm text-cyan-200 md:col-span-3">+ Add your first role</button>}</div>
        </section>
      </>}
    </main>
