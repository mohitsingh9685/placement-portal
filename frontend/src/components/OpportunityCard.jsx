...
export default function OpportunityCard({ company, applied, guest, isSaved, saving, onSave }) {
  ...
  return <article className="flex min-w-0 flex-col overflow-hidden rounded-2xl border border-white/10 bg-slate-900/80 shadow-xl shadow-black/10 transition-shadow hover:shadow-cyan-950/20">
    <div className="space-y-5 p-5 sm:p-6">
      ...
      </div> : <div className="rounded-xl bg-slate-950/45 px-4 py-3.5"><p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Compensation</p><p title={compensation} className="mt-2 line-clamp-3 whitespace-pre-wrap break-words text-sm font-medium leading-relaxed text-slate-100">{compensation}</p></div>}
    </div>
