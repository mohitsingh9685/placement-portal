...
export default function OpportunityCard({ company, applied, guest, isSaved, saving, onSave }) {
  ...
  return <article className="flex min-w-0 flex-col overflow-hidden rounded-2xl border border-white/10 bg-slate-900/80 shadow-xl shadow-black/10 transition-shadow hover:shadow-cyan-950/20">
    <div className="space-y-5 p-5 sm:p-6">
      ...
      {multipleRoles ? <div className="rounded-xl bg-slate-950/45 px-4 py-3.5">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Roles</p>
        <ul aria-label={`${company.companyName} roles`} className="mt-3 space-y-3">{roles.map((item, index) => <li key={item._id || index} className="flex items-start gap-2.5 text-sm font-medium leading-relaxed text-slate-100"><span aria-hidden="true" className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-cyan-300/70" /><span className="min-w-0 break-words">{item.title}</span></li>)}</ul>
      </div> : <div className="rounded-xl bg-slate-950/45 px-4 py-3.5"><p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Compensation</p><p title={formatCompensation(company)} className="mt-2 line-clamp-3 whitespace-pre-wrap break-words text-sm font-medium leading-relaxed text-slate-100">{formatCompensation(company)}</p></div>}
    </div>
    <div className="mt-auto px-5 pb-5 sm:px-6 sm:pb-6">
      <dl className={`grid gap-4 border-t border-white/10 pt-4 text-sm ${multipleRoles ? "grid-cols-1" : "grid-cols-[minmax(0,0.7fr)_minmax(0,1.3fr)]"}`}>
