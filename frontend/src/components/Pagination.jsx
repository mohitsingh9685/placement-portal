export default function Pagination({ data, page, onPage, loading, label = "results", compact = false }) {
  ...
  return <nav aria-label={`${label} pages`} className={`flex flex-wrap items-center justify-between gap-3 ${compact ? "mt-3 text-xs" : "mt-5 text-sm"}`}>
    ...
    <div className="flex items-center gap-3"><button disabled={loading || page <= 1} onClick={() => onPage(page > pages ? 1 : page - 1)} className={button}>Previous</button><span>Page {page} of {pages}</span><button disabled={loading || page >= pages} onClick={() => onPage(page + 1)} className={button}>Next</button></div>
  </nav>;
}
