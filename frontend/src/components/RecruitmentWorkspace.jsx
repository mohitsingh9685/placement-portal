...
const panel = "rounded-2xl border border-white/10 bg-slate-900/80 p-5 space-y-4";
const inputClass = "mt-2 block w-full rounded-xl border border-white/15 bg-slate-950 p-3 text-sm";
...
export default function RecruitmentWorkspace({ company, role, user, selectedEmails = [], onUpdated }) {
  ...
  return <section className="space-y-5">
    ...
    {hasPermission(user, "applications.export") && <details className={panel}><summary className="cursor-pointer text-lg font-semibold">Export applicants</summary>
      <fieldset disabled={busy} className="space-y-4"><p className="text-sm text-slate-400">{role ? `Exporting ${role.title}.` : "Exporting all roles in this drive."} Submitted details and approved corrections are used.</p>
        ...
        <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">{Object.entries(columns).map(([key, label]) => <label key={key} className="flex gap-2"><input type="checkbox" checked={chosen.includes(key)} onChange={e => setChosen(e.target.checked ? [...chosen, key] : chosen.filter(k => k !== key))} />{label}</label>)}</div>
        <button type="button" className={button} disabled={!chosen.length} onClick={download}>Download applicants</button>
      </fieldset>
    </details>}
    {canRounds && <section className={panel}><h2 className="text-xl font-semibold">Recruiter results</h2>{!role ? <p className="text-slate-400">Choose a role above to import or publish round results.</p> : <>
      <form onSubmit={makePreview}><fieldset disabled={busy} className="space-y-4">
        ...
        <p className="text-sm text-slate-400">Enter emails of students moving to the next round. Upload one Excel sheet or CSV with an Email column, or paste one email per line. Maximum 5,000 rows and 2 MB.</p>
        ...
        {file ? <p className="text-sm">{file.name} <button type="button" className="ml-4 text-cyan-300" onClick={() => { clearFile(); changed(); }}>Use pasted emails instead</button></p> : <label className="block">Shortlisted email addresses<textarea className={inputClass} value={text} maxLength={250000} rows={4} placeholder="student@gmail.com" onChange={e => { setText(e.target.value); changed(); }} /></label>}
        ...
        <label className="block">Message to students<textarea required minLength={3} maxLength={1000} rows={2} placeholder="e.g. Recruiter shortlist received for the next interview round." className={inputClass} value={reason} onChange={e => { setReason(e.target.value); changed(); }} /></label>
    ...
    {canRounds && <details className={panel}><summary className="cursor-pointer text-lg font-semibold">Published result history</summary>{historyError && <p role="alert">{historyError}</p>}<p className="text-xs text-slate-400">Most recent 100 batches in this drive. Undo is available only before affected applications have newer activity.</p>
