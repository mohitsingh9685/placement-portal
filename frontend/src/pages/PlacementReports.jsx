...
function ReportContent({ cohort }) {
  ...
  return <>
    ...
    <section className={panel}><h2 className="text-xl font-semibold">Offer counts</h2><div className="mt-4 flex flex-wrap gap-x-8 gap-y-3">{["issued", "accepted", "joined", "declined", "revoked"].map(s => <p key={s} className="capitalize">{s}: <strong>{report?.[s] ?? "—"}</strong></p>)}</div></section>
    <section className={panel}><h2 className="text-xl font-semibold">Placement summaries</h2><div className="mt-4 flex flex-wrap gap-2">{["branch", "year", "company"].map(g => <button key={g} aria-pressed={group === g} className={`rounded-xl border px-4 py-2 capitalize ${group === g ? "border-cyan-400 bg-cyan-500/20" : "border-white/15"}`} onClick={() => { setGroup(g); setGroupPage(1); }}>{g}</button>)}</div>
      {group === "company" && <><div className="mt-4 flex items-end gap-3"><label className="min-w-0 flex-1">Search companies<input className={field} maxLength={100} value={companySearch} onChange={e => { setCompanySearch(e.target.value); setGroupPage(1); }} /></label><ResetFiltersButton label="Reset summary filters" className="min-h-12" onClick={() => { setCompanySearch(""); setGroup("branch"); setGroupPage(1); }} /></div><p className="mt-3 text-sm text-slate-400">A student may appear under multiple companies.</p></>}
    ...
    <section className={panel}><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-semibold">Placed & unplaced students</h2><ResetFiltersButton label="Reset student filters" onClick={() => { setSearch(""); setPlacement(""); setStudentPage(1); }} /></div><div className="mt-4 grid gap-4 sm:grid-cols-2"><label>Search students<input className={field} maxLength={100} placeholder="Name, email or roll number" value={search} onChange={e => { setSearch(e.target.value); setStudentPage(1); }} /></label><label>Placement status<select className={field} value={placement} onChange={e => { setPlacement(e.target.value); setStudentPage(1); }}><option value="">All students</option><option value="PLACED">Placed</option><option value="NOT_PLACED">Unplaced</option></select></label></div>
...
export default function PlacementReports() {
  ...
  return <div className="premium-shell min-h-screen"><Navbar /><main className="mx-auto max-w-7xl space-y-6 px-4 py-8 text-slate-100">
    <header className={panel}><h1 className="text-3xl font-bold">Placement reports</h1><p className="mt-2 text-sm text-slate-400">Registered students only.</p></header>
    <section aria-label="Report filters" className={`${panel} grid gap-4 sm:grid-cols-2 lg:grid-cols-[repeat(3,minmax(0,1fr))_auto]`}>{[["course", "Course", "courses"], ["branch", "Branch", "branches"], ["passingYear", "Graduating year", "years"]].map(([key, name, values]) => <label key={key}>{name}<select className={field} value={cohort[key]} onChange={e => setCohort(old => ({ ...old, [key]: e.target.value }))}><option value="">All</option>{options.data?.[values].map(value => <option key={value} value={value}>{value}</option>)}</select></label>)}<ResetFiltersButton label="Reset report filters" onClick={resetFilters} className="min-h-12 self-end justify-self-end" /><Failure query={options} /></section>
