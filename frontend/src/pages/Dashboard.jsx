...
export default function Dashboard() {
  ...
  return <div className="premium-shell flex h-dvh !min-h-0 flex-col overflow-hidden [&>nav]:shrink-0"><Navbar wide /><main className="flex min-h-0 w-full flex-1 flex-col gap-3 overflow-hidden px-4 py-3 text-slate-100 sm:px-6 lg:px-8">
    <h1 className="sr-only">Placement opportunities</h1>
    ...
    <div className="grid min-h-0 flex-1 grid-rows-[minmax(0,1fr)] gap-5 lg:grid-cols-[250px_minmax(0,1fr)]">
      ...
      <section aria-label="Placement drives" className={`min-h-0 min-w-0 flex-col ${filtersOpen ? "hidden lg:flex" : "flex"}`}>
        <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-2">
          ...
          {!guest && <Link to="/profile" className="flex min-w-0 max-w-full items-center gap-3 rounded-xl border border-white/10 bg-slate-900/60 px-3 py-2 hover:bg-slate-800/80">
            ...
            <span className="min-w-0"><span className="block truncate text-sm font-semibold">{user?.name}</span><span className="block truncate text-xs text-slate-400">{[user?.course, user?.branch, user?.cgpa != null ? `CGPA ${user.cgpa}` : null].filter(Boolean).join(" · ")}</span></span><span aria-hidden="true" className="ml-2 text-slate-400">↗</span>
          </Link>}
        </div>
        <div ref={resultsRef} role="region" aria-label="Company results" aria-busy={loading} tabIndex={0} className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain py-3 pr-1 focus-visible:outline-cyan-300">
