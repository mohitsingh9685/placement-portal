...
export default function AdminStudents() {
  ...
  const [csv, setCsv] = useState(""); const [preview, setPreview] = useState(null); const [edit, setEdit] = useState(null);
  useEffect(() => {
    let cancelled = false;
  ...
  async function update(entry, changes) {
    ...
    try {
      ...
      setEdit(null); setRevision(value => value + 1); setNotice("Student roster updated.");
  ...
  return <div className="premium-shell min-h-screen bg-slate-100"><Navbar /><main className="mx-auto max-w-7xl space-y-6 px-4 py-8">
    <header><h1 className="text-3xl font-bold text-slate-900">Students & batches</h1><p className="mt-2 text-slate-600">Manage the college email list. Only approved students can sign in.</p></header>
    ...
    {canManage && <section className="space-y-4 rounded-2xl bg-white p-5 shadow-sm">
      ...
      <p className="text-sm text-slate-600">Paste one email per line, or use CSV columns: email, name, enrollmentNo, branch, passingYear. Existing entries keep their details and access status. Imports cannot grant admin access.</p>
